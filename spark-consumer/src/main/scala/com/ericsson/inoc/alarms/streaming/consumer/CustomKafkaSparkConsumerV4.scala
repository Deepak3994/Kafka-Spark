 ...
 * Usage: CustomKafkaSparkConsumerV4 <brokers> <topics>
 *   <brokers> is a list of one or more Kafka brokers
 *   <topics> is a list of one or more kafka topics to consume from
 *
 * Example:
 *    $ bin/run-example streaming.CustomKafkaSparkConsumerV4 
 */

object CustomKafkaSparkConsumerV4 {

    // Create context with 2 second batch interval
	
	def main(args : Array[String]) {          
	val sparkConf = new SparkConf()
	sparkConf.setAppName("CustomKafkaSparkConsumerV4")
	...
	val checkpointDirectory = "/home/neo/sources/StreamingCheckPointDir/ConsumerV4"
	//val ssc = new StreamingContext(sparkConf, Seconds(2))

		val ssc = new StreamingContext(sparkConf, Seconds(2))   // new context
		
	    // Create direct kafka stream with brokers and topics
		val topics = "sumTopic,any-topic,this-topic"
